...
```text
https://shoefitcare-chatbot.onrender.com/s/1
```

(동일: `/c/SR266`) → `/pilot` + 쿠팡 `return_url`. 매핑: `pilot_links.py` · `GET /health/build` → `pilot_short_codes`

**더 짧게 보이게:** Render **커스텀 도메인**(예: `fit.엄마신발.kr/s/1`) 또는 **bit.ly 등**에 `/s/1` 을 연결 (도메인 길이는 호스팅·단축 서비스에서만 줄일 수 있음).
